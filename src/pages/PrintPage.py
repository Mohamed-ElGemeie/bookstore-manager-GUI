...
class PrintPage(tk.Frame):
  ...
  * write the transaction details into the books table from db
  ...
  def __init__(self,parent,stacker,bg=None,fg=None):
    ...
    #buttons
    self.buy_b=tk.Button(self,font=('Helvetica',20,'bold'),text=('Confirm'),command=lambda:buy_book())
    self.print_b=tk.Button(self,font=('Helvetica',20,'bold'),text=('Confirm'),command=lambda:buy_paper())
    self.go_back=tk.Button(self,font=45,text=('Go Back <(----'),command=lambda: stacker.show_frame('startpage'))
    ...
    #placments
    self.go_back.pack(side='bottom')
    self.buy_b.place(x=150,y=320)
    ...
    self.print_b.place(x=900,y=170)
